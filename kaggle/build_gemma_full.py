"""Build gemma9b-full: full-data, checkpoint-resumable Gemma-2-9B QLoRA training.

Trains on ALL 57K rows across multiple Kaggle sessions. Each session trains until a
wall-clock time budget, force-saves a checkpoint, and commits. The next session attaches
the previous session's output as a dataset and resumes from the latest checkpoint.

Run: ../.venv/bin/python build_gemma_full.py
"""
import json
import pathlib
...
CELLS = [
    ("markdown",
     ...
     "Trains on **all 57K rows** across multiple Kaggle sessions (the 9h limit can't fit one "
     "full epoch of 9B on T4 x2). Each session trains up to a time budget, force-saves a "
     "checkpoint, and you commit. The next session **attaches the previous output** and resumes.\n\n"
     "### Multi-session workflow\n"
     "1. **Session 1:** Accelerator = **GPU T4 x2**, Internet **on**. Model + competition auto-attach. "
     "Save & Run All. It trains ~7.5h, saves checkpoints to `/kaggle/working/ckpt`, commits.\n"
     "2. From that version's **Output**, create a **Kaggle Dataset** (e.g. `gemma-ckpt`).\n"
     "3. **Session 2+:** **Add Input** that `gemma-ckpt` dataset. Run again — it auto-detects the "
     "latest checkpoint and **resumes**. Commit, then **update the dataset** with the new output.\n"
     "4. Repeat until it prints `TRAINING COMPLETE` (epoch finished). The final adapter is at "
     "`/kaggle/working/adapter` — use it in the `gemma-infer` notebook."),
    ("code",
...
meta = {
    "id": f"{USER}/gemma9b-full", "title": "gemma9b-full",
    "code_file": "gemma9b-full.ipynb", "language": "python", "kernel_type": "notebook",
    "is_private": True, "enable_gpu": True, "enable_internet": True,
    "dataset_sources": [], "competition_sources": ["llm-classification-finetuning"],
    "model_sources": [GEMMA_MODEL], "kernel_sources": [],
}

d = HERE / "gemma9b-full"
d.mkdir(exist_ok=True)
nbf.write(build(), d / "gemma9b-full.ipynb")
(d / "kernel-metadata.json").write_text(json.dumps(meta, indent=2))
print("wrote", d / "gemma9b-full.ipynb", "and its kernel-metadata.json")
